...
//		      |                 | nOpce = 4 - alteração
// -----------+-----------------+-----------------------------------------------------------
User Function TOTA0012()

	Local cLinok 	     := "ExecBlock('LinOk',.f.,.f.)"
	Local cTudook 	     := "ExecBlock('TudOk',.f.,.f.)"
	...
	Private cCadastro	 := "Pedido de Livros - Alteração"
	Private aCols 		 := {}
	Private aHeader 	 := {}
	Private aCpoEnchoice := {}
	Private aAltEnchoice := {"ZZ4_VEND","ZZ4_NOMVED","ZZ4_VLTOT"}
	Private cTitulo
	Private cAlias1 	 := "ZZ4"
	Private cAlias2 	 := "ZZ5"
	...
	if lRet
		cMensagem += "Esta rotina tem a finalidade de salvar alguns campos no PEDIDO DE COMPRAS LIBERADO"+CRLF+CRLF
...
Static function DefineaCols(nOpc)
	...
	Local aZZ5		:= {"ZZ5_CODPED", "ZZ5_ITEM", "ZZ5_CODLIV", "ZZ5_DESCLI", "ZZ5_QUANT", "ZZ5_PRECO", "ZZ5_TOTAL"}
	Local nQtdcpo 	:= 0
	Local i			:= 0
	Local nCols 	:= 0
	Local nUsado	:= 0
...
Static Function Gravar()
	...
	Local nI
	Local cCamposZZ4 := ""
	Local cCamposZZ5 := "ZZ5_CODPED;ZZ5_ITEM;ZZ5_CODLIV;ZZ5_DESCLI;ZZ5_QUANT;ZZ5_PRECO;ZZ5_TOTAL"
	Local nCod
